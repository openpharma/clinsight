...
get_review_data <- function(
  ...
  all_review_data <- data |> 
    ...
    dplyr::select(dplyr::all_of(c(key_cols, date_var, edit_time_var))) |> 
    ...
    dplyr::slice_max(.data[[edit_time_var]], by = dplyr::all_of(key_cols), with_ties = FALSE) |> 
...
update_review_data <- function(
  ...
  df <- df |> 
    ...
    dplyr::mutate(
      ...
      .by = dplyr::all_of(c(key_cols))
    )
  updated_data <- dplyr::anti_join(df, review_df, 
                                   by = c(key_cols, edit_time_var))
...
summarize_review_data <- function(
    data, 
    key_cols = c("subject_id", "item_group"),
  ...
  if(dplyr::pull(dplyr::tally(data), n) != 0){
    data <- data |> 
      dplyr::select(dplyr::all_of(c(key_cols, event_var, date_time_vars, status_var))) |> 
