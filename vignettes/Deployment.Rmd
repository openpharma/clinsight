...
-   `allow_to_query`: List of role ids (see `user_roles`) which are allowed to raise a query and close a query. Only effective if `allow_query_inputs` is `TRUE`. Note that every user with a valid name and role can respond to a query, regardless of their role being listed here.
