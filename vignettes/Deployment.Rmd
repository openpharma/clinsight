...
-   `app_prod`: Controls a few production-related settings, such as whether a warning is shown if the data is not synchronized, or whether the database is always updated. It is recommended to set this to `TRUE` when the application is run in production.
...
-   `study_data`: In the default settings, this path will directly return the internal package data for testing purposes. In production, this argument should contain the file path to the study data `.rds` file.
-   `meta_data`: In the default settings, this path will return example metadata for testing purposes. In production, this argument should contain the file path to the metadata `.rds` file.
-   `user_db`: Path to the user database file.
...
-   `allow_to_review`: List with roles which are allowed to review data in the application.
-   `allow_listing_download`: Logical, whether it is allowed to download Excel tables from the listings shown in each form.
-   `allow_query_inputs`: Logical, whether it is allowed to create queries within the application.
...
In `vignette('clinsight')` the function `test_clinsight()` was used to run the application, which is a wrapper around `clinsight::run_app()`.
`test_clinsight()` ensures that the required databases are created every time after closing the application.
This makes it easier to prototype the application, and tweak the data/metadata for use with ClinSight, since the databases will be always up to date with the study_data and metadata.

However, for deployment the function `clinsight::run_app()` should be used.
This creates databases in the working directory that will save user actions such as review status of study data.
Depending on the settings, a credentials database will be created.
In addition, a path study data and metadata can be provided in the configuration file.
...
To use it, set the active configuration to `posit_connect`:`Sys.setenv("GOLEM_CONFIG_ACTIVE" = "posit_connect")`. This configuration utilizes Shiny `session` objects to manager user access and credentials. The `user_role` element of `config.yml` will correspond to `session$groups` so that users can be assigned review status by group assignment within Connect. The `session` object `session$user` will be utilized to log user interactions in the application as well.

For ease of deployment, a `manifest.json` file has been included in the root directory. This manifest includes packages not included in the `minimal` profile needed for deployment using the `app.R` file.
