...
Within the first tab, the column names of the raw data need to be specified. The columns with the names within `name_raw` will be renamed to the new name when merging metadata with raw data later. Again, orange fields should not be adjusted. These field contain `ClinSight` [key_columns()], defining unique records, and other essential variables such as `edit_date_time`, defining the last time a data point was edited.
...
By default, a subject's study status is derived from variables in the general tab (`Eligible`, `DiscontinuationReason`, `DiscontinuationDate`). To override this, add a subject_status column to the study data—if present, it will be used directly and no calculation will occur. See below for example code to implement this.
...
In the section above the function `calculate_subject_status` contains some custom logic that calculates the needed status category per patient, based on other variables in the data. 
