...
Then, ensure that the shinyproxy configuration .yml file is setup correctly. Below is an example to get started. Note that these settings assume that openID Connect is used for authentication. Configuring this properly is outside of the scope of this guide; see the [shinyproxy](https://shinyproxy.io/) documentation for details on this.
...
- User names are passed through using HTTP headers. Since HTTP headers cannot handle all UTF-8 characters, the user names need to be base64 encoded as shown in the example `.yml` file.
