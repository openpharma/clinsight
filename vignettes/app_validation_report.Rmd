...
1.  The application is developed within Mozilla Firefox (v121.0) and
    validated with Google Chrome (v120.0.6099.199). Although all major
    ...
    than Mozilla Firefox and Google Chrome.
...
-   Risk mitigation: We implement three layers of security to protect
    against unauthorized access: first, the user needs access to our
    access-restricted VPN. Then, the user needs to login into the
    virtual machine. Lastly, the user needs to sign in into the
    application on the virtual machine. This is documented in the
...
This package is used because it helps to implement a login system and a
credential management layer within the application, while the
credentials database can be stored locally. No better alternatives that
allow a locally stored credentials database are known as of writing of
this validation report.

#### Risk mitigation

The risk of using this package is mitigated by adding an additional
security layer: the application will be deployed behind our local VPN,
ensuring that the application can only be reached by authorized
personnel. The `shinymanager` package will be used as a second
authentication system to enable identification of the application users.
This mitigates the risk of the package use.
...
package manager with a snapshot of the public CRAN repository on
11November 2023 (https://packagemanager.posit.co/cran/2023-11-01).
