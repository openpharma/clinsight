...
# create_table.general() works: creates expected table
    ...
    Output
      ...
      #   DrugDiscontReason <chr>, subject_status <chr>, status_label <chr>
...
# create_table.adverse_events() works: creates expected AE table
    ...
    Output
      ...
      # i 65 more rows
      # i 9 more variables: `Treatment related` <chr>, `Treatment action` <chr>,
      #   `Serious Adverse Event` <chr>, `SAE Start date` <chr>,
      #   `SAE End date` <chr>, `SAE Category` <chr>, `SAE Awareness date` <chr>,
      #   `SAE Date of death` <chr>, `SAE Death reason` <chr>
