describe(
  "fix_multiple_choice_vars() works", 
  {
    ...
    it("gives the expected output", {
      expect_true(is.data.frame(fix_multiple_choice_vars(df, keys = "ID")))
      expected <- data.frame(
        ID = "Subj1",
        var = c("Age", "MH_TRT"),
        item_value = c("95", "67; 58; 83; 34")
      )
      expect_equal(fix_multiple_choice_vars(df, keys = "ID"), expected)
    })
    it("returns the same df if no mc vars are found", {
      ...
      expect_equal(fix_multiple_choice_vars(df, keys = "ID"), df)
    })
